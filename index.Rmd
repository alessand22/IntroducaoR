...
No R Studio, para exibir os pacotes instalados, alternativamente, pode-se simplesmente clicar na aba **Packages**, no canto inferior esquerdo.
...
```
O RStudio oferece uma segunda opção de carregar pacote por meio da aba **Packages**, no canto inferior direto. Clique nessa aba que serão exibidos os pacotes instalados e marque o pacote que deseja carregar.
...
Exitem, no entanto, diversos pacotes para os mais variados fins. Para utilizar os recursos desses pacotes, eles devem ser primeiramente instalados e, posteriormente, carregados.

Os pacotes ficam armazenados em repositórios (*repository*), detro os quais, os mais populares são:
...
No RStudio, há três maneiras de instalar pacotes: duas da internet, (a) digitando o comando `install.packages()` (igual no R básico), (b) por meio da aba `Packages` e (c) a partir de um arquivo armazenado localmente.


#### Instação de pacotes do CRAN
A primeira, envolve  a digitação do comando de instalação de um pacote cuja sintaxe é a seguinte: 
...
O pacote pomite ainda realizar a importação por meio de código. a sintaxe da função é:

```{r, eval=FALSE}
read_excel("caminho do arquivo/arquivo.xlsx")
```

A seguir, são apresentados quatro exemplos, o primeiro supondo que o arquivo do Excel está no diretório de trabalho, e, portanto, sem a necessidade de escrever o cominho; o segundo supondo que o arquivo está num subdiretório do diretório de trabalho; o terceiro com o caminho completo do arquivo; e o quarto, supondo que o arquivo esteja no diretório de trabalho, mas evidenciando alguns argumentos, como a posição da aba no Excel que, no caso, é a primeira (`sheet = 1`), que a primeira linha possui os nomes das variáveis (`col_names = TRUE`) e que os dados ausentes são indicados por células vazias (`na = ""`).
...
Você pode observar que o arquivo **.Rdata** é capaz de armazenar diversos objetos que são carregados no ambiente de trabalho. A seção 4.5 trata com mais detalhes esse essunto.

No exemplo, os arquivo está no diretório de trabalho;  o entanto, caso esteja noutro diretório, deve-se incluir o caminho completo no comando `load("C:/usuarios/nome/pasta/desc_morettin.Rdata")`. Atenção que um erro muito comum entre os usuários iniciantes é o esquecimento das aspas.
...
Uma utilidade interessante do comando `dump()` é a chamada de funções, que são sequências de instruções criadas no R. O usuário pode criar uma função e salvar o script para automatizar alguma rotina.

Vamos chamar uma função simples criada para contar a quantidade de elementos num vetor e calcular a média. Denominamos essa função de [**minhaFuncao.R**](minhaFuncao.R). A seguir, o exemplo:
...
6. Há algum outro comando que poderia realizar a terefa da questão anterior além do comando `dump()`? Qual?
7. Importe os dados no arquivo [**cimento.Rdata**](dados/cimento.Rdata)


# Gráficos

O R possui poderosos e vastos recursos Gráficos que possibilitam incontáveis variações e possibilidades de análise de dados. Nesta seção, serão demonstrados os procedimentos básicos para elaboração de gráficos; todavia, há pacotes adicionais, como o `ggplot2` capazes de aumentar a capacidade de elaboração gráfica. O `ggplot2` será apresentado numa outra seção.

## Comando plot

O comando `plot(x,y,...)` pode criar gráficos a partir de uma única série de dados ou da combinação de dois conjuntos no forma de coordenadas x-y. A seguir, são exemplificados as duas situações.
...
Há seis tipos de linhas (`type="l"`) disponíveis e configuráveis pelo argumento `lty` listadas a seguir.
...
É possível plotar mais uma linha no mesmo gráfico. No exemplo a seguir, dois conjuntos de dados gerados aleatoriamente com o comando `runif()` e são plotados no mesmo gráfico. O comando `set.seet()` gera uma semente que força os resultados "aleatórios" serem iguais aos aqui demonstrados; caso contrário, os resultaos gerados por `runif()` serão diferentes. O argumento `ann=F` suprime os rótulos dos títulos e eixos e `axes=F` suprime as marcas (*tick marks*) dos eixos de x e y do segundo gráfico.
...
Diversos componentes podem ser acrescentados aos gráficos em geral, como linhas, objetos, setas e textos. No quadro abaixo, listam-se os principais.  
...
Os argumentos opcionais definem cores, tamanho e outras características dos elementos adicionados. Esses componentes podem ser demonstrados num gráfico em que são acrescentados livremente apenas para demonstração.
...
O `barplot` faz uma parceria muito útil com as tabelas como no exemplo a seguir criadas com o comando `table()` e preenchidas com o argumento `density =`:
