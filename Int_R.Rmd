...
O sítio ofical do R o R project (https://www.r-project.org/), onde possível encontrar inúmeras informações sobre o ambiente, literatura, contribuições em diversas formatos.
...
O RStudio é uma interface de desenvolvimento integrada (IDE) que atua conjudado com o R, facilitando as operações e integrações com outros recursos como o Rmardown. Possibilita desde o preenchimento automático de palavras (o que ajuda muito), criação de relatórios em HTML. PDF, Word e PowerPoint, e criação de novos pacotes.


Para a sua intação, siga as instruções abaixo:
...
Uma maneira mais prática de imprtação de dados é utilizando o pacote **readxl** que permite importação de dados de dados armazenados em diversos formatos, em txt, Excel, SPSS, SAS e STATA.

Para importar dados em planilhas do Excel na janela, deve-se clicar na botão **Import Dataset**, na aba **Environment** localizada na janela superior direita do terminal do RStudio, e selionar **From Excel...**


![Estruturas de dados em R](./figuras/importReadxl.png){ width=50% }
<br>

Um janela de naveção, é aberta para encontrar o arquivo e abri-lo. O exemlo buscamos o arquivo **aditivo.xlsx** que está no diretório de trabalho atual.
...
A seguir um exemplo, primeiro supondo que o arquivo Excel está no diretório de trabalho, e sem a necessidade de escrever cominho; un segundo supondo que o arquivo está num subdiretório do diretório de trabalho; e um terceiro com o caminho completo do arquivo.
...
Para importar dados do R com as extensões **.R**, **.rda** ou **.Rdata**, basta utilizar o comando `load()`. Por exemplo para importar os dados do arquivo "desc_morettin.Rdata".
...
Uma utilidade interessante do comando `dump()` é a chamada de funções, que são sequências de instruções, criadas no R. Vamos chamar uma função simples, `minhaFuncao.R` que conta os quantidade de elementos num vetor e calcular a média.
...
2. Importe os dados dos ensaios de vida útil de baterias contidos na aba **bateria** do arquivo **dadosPPE1.xlsx**. não deixe de importar o rótulo da coluna.
...
7. Importe os dados no arquivo **cimento.Rdata**
